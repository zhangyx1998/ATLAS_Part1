...
def version_control():
  if Debug:
    print("VER_CTRL >> In Version Control")
  #try:
  if Debug: print("VER_CTRL >> Connecting to Database using "+L_Key.Host+' '+L_Key.ID+' '+L_Key.PW+' '+L_Key.DB)
  db=MySQLdb.connect(L_Key.Host, L_Key.ID, L_Key.PW)
  cursor = db.cursor()
  SQL_CMD="use "+L_Key.DB
  cursor.execute(SQL_CMD)
  cursor.execute("SELECT MSG_Index From Log WHERE MSG_Type='Current_Version'")
  line = cursor.fetchone()
  prev_version=line[0]
  if Debug:
    print("VER_CTRL >> Previous version: "+prev_version)
  if (version!=prev_version):
    if Debug:
      print("VER_CTRL >> Update Detected: "+version)
    SQL_CMD="INSERT INTO "+log_table+" (MSG_Source, MSG_Type, MSG_Index) VALUES('VERSION', 'Version_Update','"+version+"');"
    cursor.execute(SQL_CMD)
    SQL_CMD="UPDATE "+log_table+" SET MSG_Index='"+version+"' WHERE MSG_Type='Current_Version'"
    print SQL_CMD
    cursor.execute(SQL_CMD)
  db.commit()
  db.close()
  #except:
  #  report_MSG("ERROR","Could Not Verify Version",1,2,"Arduino_IO")
  #  sys.exit(0)
